...
function Button({
	...
	const btnBorder = (variant) => {
		if (variant === "accent") {
			return {
				...
				borderTopColor: dark
					? `oklch(0.65 0.18 ${t.hue})`
					: "rgba(255,255,255,0.3)",
				borderLeftColor: dark
					? `oklch(0.65 0.18 ${t.hue})`
					: "rgba(255,255,255,0.25)",
				borderRightColor: dark
					? `oklch(0.40 0.14 ${t.hue})`
					: "rgba(0,0,0,0.08)",
				borderBottomColor: dark
					? `oklch(0.38 0.12 ${t.hue})`
					: "rgba(0,0,0,0.1)",
			};
		}
		return {
	...
	const variants = {
		primary: {
			background: `oklch(0.88 0.005 ${t.hue})`,
			color: t.pageBg,
			...btnBorder("primary"),
			boxShadow: raised,
		},
	...
	return (
		...
		>
			<div
				style={{
					padding: "14px 16px",
					borderBottom: `1px solid ${dark ? `oklch(0.24 0.006 ${t.hue})` : t.border}`,
					display: "flex",
					justifyContent: "space-between",
					alignItems: "center",
				}}
			>
				<span style={{ fontSize: 14, fontWeight: 600, color: t.fg1 }}>
					{title}
				</span>
				<span
					onClick={onClose}
			...
			{rows.map((row, ri) => (
				<div
					...
					style={{
						...
						borderBottom:
							ri < rows.length - 1
								? `1px solid ${dark ? `oklch(0.22 0.005 ${t.hue})` : t.border}`
								: "none",
					}}
				>
					{row.map((cell, ci) => (
